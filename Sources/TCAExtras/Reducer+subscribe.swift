...
// Adapted from: https://github.com/oliverfoggin/swift-composable-subscriber
...
enum Stream<State, Value> {
  ...
  @usableFromInline
  func callAsFunction(state: State) async -> AsyncStream<Value> {
    switch self {
...
public struct _SubscribeReducer<Parent: Reducer, TriggerAction, StreamElement, Value>: Reducer {
  ...
  public func reduce(into state: inout Parent.State, action: Parent.Action) -> Effect<Parent.Action> {
    ...
    guard triggerAction.extract(from: action) != nil else {
      return effects
    }
